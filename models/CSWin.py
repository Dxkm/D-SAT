...
from models.Attention import SSA
...
class CSWinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, in_channels=3,
                 embed_dim=64, depth=[1, 2, 21, 2], split_size=[1, 2, 7, 7], num_heads=[2, 4, 8, 16],
    ...
    def forward(self, x):
        x = self.stage1_conv_embed(x)   # (B, 3136, 64)
        # Stage1
        for blk in self.stage1:
            x = blk(x)
        stage1_x_t = x.clone()          # (B, 3136, 64)

        x = self.merge1(x)
        # Stage2
        for blk in self.stage2:
            x = blk(x)
        stage2_x_t = x.clone()          # (B, 784, 128)

        x = self.merge2(x)
        # Stage3
        for blk in self.stage3:
            x = blk(x)
        stage3_x_t = x.clone()          # (B, 196, 256)

        x = self.merge3(x)
        # Stage 4
        for blk in self.stage4:
            x = blk(x)
        stage4_x_t = x.clone()          # (B, 49, 512)

        t_feature = [stage1_x_t, stage2_x_t, stage3_x_t, stage4_x_t]
